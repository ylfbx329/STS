...
from patches import downPixelAggr, save_test_patches, save_random_patches, save_random_patches60, save_test_patches60
...
def readS2fromFile(data_file,
    ...
    for b in range(0, ds10.RasterCount):
        desc = validate_description(ds10.GetRasterBand(b + 1).GetDescription())  # 转成'B4 (665 nm)'形式
        shortname = get_band_short_name(desc)  # 获取名字 B4
    ...
    if validated_10m_indices:
        ...
        arr10 = ds10.ReadAsArray(xoff=xmin, yoff=ymin,
                                 xsize=xmax - xmin + 1, ysize=ymax - ymin + 1,
                                 buf_xsize=xmax - xmin + 1, buf_ysize=ymax - ymin + 1)  # (c,h,w)
        data10 = np.rollaxis(arr10, 0, 3)[:, :, validated_10m_indices]  # (h,w,c)

    # if validated_20m_indices:
    #     print("Loading selected data from: %s" % selected_20m_data_set[1])
    #     arr20 = ds20.ReadAsArray(xoff=xmin // 2, yoff=ymin // 2,
    #                              xsize=(xmax - xmin + 1) // 2, ysize=(ymax - ymin + 1) // 2,
    #                              buf_xsize=(xmax - xmin + 1) // 2, buf_ysize=(ymax - ymin + 1) // 2)
    #     data20 = np.rollaxis(arr20, 0, 3)[:, :, validated_20m_indices]
    #
    # if validated_60m_indices:
    #     print("Loading selected data from: %s" % selected_60m_data_set[1])
    #     arr60 = ds60.ReadAsArray(xoff=xmin // 6, yoff=ymin // 6,
    #                              xsize=(xmax - xmin + 1) // 6, ysize=(ymax - ymin + 1) // 6,
    #                              buf_xsize=(xmax - xmin + 1) // 6, buf_ysize=(ymax - ymin + 1) // 6)
    #     data60 = np.rollaxis(arr60, 0, 3)[:, :, validated_60m_indices]
...
parser = argparse.ArgumentParser(description="Read Sentinel-2 data. The code was adapted from N. Brodu.",
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument("-data_file", default="../../data/sentinel/S2A_MSIL1C_20160929T170302_N0204_R069_T15RTN_20160929T171011.SAFE",
...
parser.add_argument("--save_prefix", default="../data/",
...
parser.add_argument("--true_data", default=False, action="store_true",
